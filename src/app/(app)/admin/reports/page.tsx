...
/** Reconciliation view: filter, read on screen, or download the same rows as a file. */
export default function ReportsPage() {
  const [kind, setKind] = useState<Kind>("collections");
  const [draft, setDraft] = useState<ReportFilter>(EMPTY);
  ...
  return (
    <>
      <PageHeader
        title="Reports"
        description="Reconcile what moved, by tenant and date"
        actions={
          <>
      ...
      <Tabs value={kind} onValueChange={(v) => setKind(v as Kind)}>
        <TabsList>
          ...
          <TabsTrigger value="disbursements">Payouts</TabsTrigger>
        </TabsList>
      </Tabs>
        ...
        <PanelBody>
          ...
          >
            ...
            <Field label="From" htmlFor="r-from">
              <Input id="r-from" type="date" value={draft.fromDate ?? ""} onChange={(e) => setDraft({ ...draft, fromDate: e.target.value || undefined })} />
            </Field>
            <Field label="To" htmlFor="r-to">
              <Input id="r-to" type="date" value={draft.toDate ?? ""} min={draft.fromDate} onChange={(e) => setDraft({ ...draft, toDate: e.target.value || undefined })} />
            </Field>
      ...
      <BalanceStrip
        heroLabel={kind === "collections" ? "Collected" : "Paid out"}
      ...
      <Ledger
        ...
        emptyDescription="Widen the date range, or choose a different tenant."
